...
function addActions(_ref, actions) {
  var type = _ref.type;

  if ((0, _scheduling.schedulingEnabled)(type)) {
    return [_schedule.scheduleAction, _schedule.unScheduleAction, CustomPublishAction, CustomDeleteAction].concat(actions.filter(function (action) {
      return !['DeleteAction', 'PublishAction'].includes(action.name);
    }));
  }
